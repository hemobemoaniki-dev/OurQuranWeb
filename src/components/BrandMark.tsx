...
// Browser-tab icons deliberately use the exact winged-book mark with no
// replacement glyph. Dark browser chrome gets the transparent original. Light
// browser chrome uses the same mark with a dark halo so the white pages do not
// disappear against a light tab strip.
export const BRAND_TAB_ICON_DARK_URI = BRAND_MARK_URI;

export const BRAND_TAB_ICON_LIGHT_URI = `data:image/svg+xml;utf8,${encodeURIComponent(`
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
  <defs>
    <filter id="tabHalo" x="-45%" y="-45%" width="190%" height="190%">
      <feDropShadow dx="0" dy="1" stdDeviation="2.7" flood-color="#060606" flood-opacity=".96"/>
      <feDropShadow dx="0" dy="0" stdDeviation="1.2" flood-color="#D6A52A" flood-opacity=".55"/>
    </filter>
  </defs>
  <image href="${BRAND_MARK_URI}" x="4" y="18" width="120" height="92" preserveAspectRatio="xMidYMid meet" filter="url(#tabHalo)"/>
</svg>`)}`;

// Backward-compatible alias used by any remaining callers.
export const BRAND_FAVICON_URI = BRAND_TAB_ICON_LIGHT_URI;
