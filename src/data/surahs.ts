// Static Qur'an surah metadata (Hafs). Used for the picker, progress bar and
// previous/next boundaries. Ayah text/translation is fetched from AlQuran Cloud.
export type SurahMeta = { number: number; name: string; ayahs: number };
...
const RAW: [string, number][] = [
  ...
  ["As-Saff", 14], ["Al-Jumu'ah", 11], ["Al-Munafiqun", 11], ["At-Taghabun", 18],
...
export const SURAHS: SurahMeta[] = RAW.map(([name, ayahs], i) => ({
  number: i + 1,
  name,
  ayahs,
}));

export function surahMeta(n: number): SurahMeta {
  return SURAHS[n - 1];
}
