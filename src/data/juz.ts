// Juz boundaries: [juzNumber, startSurah, startAyah]. Used to derive the Juz of
// the current verse from real metadata (never hardcoded).
const JUZ_STARTS: [number, number, number][] = [
  [1, 1, 1], [2, 2, 142], [3, 2, 253], [4, 3, 93], [5, 4, 24], [6, 4, 148],
  [7, 5, 82], [8, 6, 111], [9, 7, 88], [10, 8, 41], [11, 9, 93], [12, 11, 6],
  [13, 12, 53], [14, 15, 1], [15, 17, 1], [16, 18, 75], [17, 21, 1], [18, 23, 1],
  [19, 25, 21], [20, 27, 56], [21, 29, 46], [22, 33, 31], [23, 36, 28], [24, 39, 32],
  [25, 41, 47], [26, 46, 1], [27, 51, 31], [28, 58, 1], [29, 67, 1], [30, 78, 1],
];

export function juzForAyah(surah: number, ayah: number): number {
  let juz = 1;
  for (const [j, s, a] of JUZ_STARTS) {
    if (surah > s || (surah === s && ayah >= a)) juz = j;
    else break;
  }
  return juz;
}
